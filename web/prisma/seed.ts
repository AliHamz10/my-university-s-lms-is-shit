...
async function main() {
  ...
  await prisma.$transaction(async (tx) => {
    ...
    const testAdmin = await tx.profile.upsert({
    ...
    // Create departments
    const csDept = await tx.department.upsert({
      where: {
        orgId_code: {
          orgId: testOrgId,
          code: 'CS',
    ...
    const mathDept = await tx.department.upsert({
      where: {
        orgId_code: {
          orgId: testOrgId,
          code: 'MATH',
    ...
    // Create terms
    const springTerm = await tx.term.upsert({
      where: {
        orgId_code: {
          orgId: testOrgId,
          code: 'SP25',
    ...
    const fallTerm = await tx.term.upsert({
      where: {
        orgId_code: {
          orgId: testOrgId,
          code: 'FA25',
    ...
    // Create courses
    const cs101 = await tx.course.upsert({
      where: {
        orgId_code: {
          orgId: testOrgId,
          code: 'CS101',
    ...
    const math101 = await tx.course.upsert({
      where: {
        orgId_code: {
          orgId: testOrgId,
          code: 'MATH101',
    ...
    const cs101Offering = await tx.courseOffering.upsert({
      where: {
    ...
    const math101Offering = await tx.courseOffering.upsert({
      where: {
        courseId_termId_sectionCode: {
          courseId: math101.id,
          termId: springTerm.id,
    ...
    const testStudent = await tx.profile.upsert({
    ...
    const assignment1 = await tx.assignment.upsert({
